...
class BK():
    ...
    def Backtracking(self,X_cur,X_goal,path,turn,visited,huong_ht,lan_lap = 0 ):
      if X_cur == X_goal:
          return path.copy()
      if lan_lap > 15:
          return None

      Tap_Gia_Tri = self.init_Tap_Gia_Tri(X_cur[0],X_cur[1])
      for (dx,dy) in Tap_Gia_Tri:
          ...
          if  self.init_Tap_Rang_Buoc(turn,dx,dy,self.ma_tran[X_goal[0]][X_goal[1]],huong_trc,huong_moi) and ((dx,dy) not in visited) :
              ...
              result = self.Backtracking((dx,dy),X_goal,path,new_turn,visited,huong_moi,lan_lap)
